...
| BTS7960 R_EN | GPIO26 | |
| BTS7960 L_EN | GPIO27 | |
| BTS7960 RPWM | GPIO25 | Driven digital HIGH/LOW (full speed, timed movement — no PWM needed). RPWM/LPWM are swapped vs. the driver's silkscreen so software "open"/"close" match the physical door direction — see `DoorController::run()`. |
| BTS7960 LPWM | GPIO33 | |
...
Power: a single **5.5 V battery** is the only supply. It feeds three things
directly — the ESP32's `VIN` (dropped to 3.3 V by the board's on-board
regulator), the BTS7960's logic `VCC`, and the BTS7960's motor rail
(`B+`/`B-`). The **only** load on the ESP32's regulated 3.3 V output is the
DS3231. All grounds are common (battery `-` ties to the ESP32 `GND` and the
BTS7960). Add bulk capacitance (1000–2200 µF) near the BTS7960's motor input
to absorb stall-current sag and avoid browning out the ESP32 mid-move.
